...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const checkUser = async () => {
      ...
      if (token) {
        try {
          const response = await fetch(`${process.env.NEXT_PUBLIC_AI_BACKEND_URL || 'https://pulsar-ai.site'}/auth/me`, {
            headers: {
    ...
    try {
      ...
      for (const attempt of jsonAttempts) {
        try {
          ...
          response = await fetch(`${process.env.NEXT_PUBLIC_AI_BACKEND_URL || 'https://pulsar-ai.site'}/auth/login`, {
            method: 'POST',
      ...
      if (token) {
        try {
          ...
          const meResponse = await fetch(`${process.env.NEXT_PUBLIC_AI_BACKEND_URL || 'https://pulsar-ai.site'}/auth/me`, {
            method: 'GET',
            headers: {
              'Authorization': `Bearer ${token}`,
  ...
  const register = async (registerData: RegisterData) => {
    ...
    try {
      ...
      try {
        response = await fetch(`${process.env.NEXT_PUBLIC_AI_BACKEND_URL || 'https://pulsar-ai.site'}/auth/register`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
